"""Generate the finvariant-mcp architecture figure (academic style).

How an AI agent calls the server, which routes to the validated finvariant core
...
ax.text(3, 96, "finvariant-mcp", fontsize=13.5, fontweight="bold", color=INK,
        ha="left")
ax.text(3, 91, "validated statement-integrity checks for AI agents",
...
ax.text(49, 79, "finvariant-mcp server  (stdio)", ha="center", fontsize=9.5,
        color=MUT, fontweight="bold")
box(49, 64, 40, 16,
    "check_statements\nverify income statement, balance sheet\n"
    "and cash flow against the invariants",
    ANA_F, ANA_E, fs=7.9)
box(49, 45, 40, 14,
    "describe_schema\ncanonical fields · sign convention · checks",
    OPT_F, OPT_E, fs=7.9)
...
ax.text(48, 22.5, "results to the agent:  structured JSON "
        "(verdict · findings, expected vs actual · counts · provenance)",
...
# banner
box(50, 8, 94, 5,
    "the agent interprets   ·   validated code checks   ·   every result "
    "carries provenance",
